...
{
    public partial class Tream_Tree : System.Web.UI.Page
    {
        ...
        private void LoadChildNodes(TreeNode PNode, int PID)
        {
            ...
            else if (PNode.Value == SysClass.SysOrgan.WorkshopType_Value.ToString())
            {
                _OrganType = SysClass.SysOrgan.DeptType_Value;
            }
            ...
            while (sdr.Read())
            {
                ...
                if (sdr["OrganType"].ToString() != "2")
                {
                    tnNode.SelectAction = TreeNodeSelectAction.None;
                }
                else
                {
                    tnNode.NavigateUrl = "Tream_Lst.aspx?POrganID=" + tnNode.Value;
                }
