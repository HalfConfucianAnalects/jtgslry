...
namespace JtgTMS.Admin
{
    public partial class Dept_Edit : System.Web.UI.Page
    {
        ...
        private void BindPageData()
        {
            ...
            txtOrganName.ReadOnly = _OrganID == 58;
            txtOrganNo.ReadOnly = _OrganID == 58;
        }
        ...
        private bool SaveCheck()
        {
            ...
            {
                bFlag = false;
                Dialog.OpenDialogInAjax(txtOrganNo, "部门编号不能为空！");
            }
            else if (SysClass.SysOrgan.CheckOrganNoExists(_OrganID, txtOrganNo.Text))
            {
                bFlag = false;
                Dialog.OpenDialogInAjax(txtOrganName, "部门编号不能重复！");
            }
            else if (txtOrganName.Text.Length == 0)
            {
                bFlag = false;
                Dialog.OpenDialogInAjax(txtOrganName, "部门名称不能为空！");
            }
            return bFlag;
        }
        ...
        protected void btnApply_Click(object sender, EventArgs e)
        {
            if (SaveCheck())
            {
                ...
                {
                    Dialog.OpenDialogInAjax(upForm, "恭喜您，保存信息成功……", "Dept_Lst.aspx?POrganID=" + _POrganID.ToString());
                }
            }
        }
        ...
        {
            Response.Redirect("Dept_Lst.aspx?POrganID=" + _POrganID.ToString());
        }   
    }
}
