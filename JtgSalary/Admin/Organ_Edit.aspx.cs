...
{
    public partial class Organ_Edit : System.Web.UI.Page
    {
        ...
        private bool SaveCheck()
        {
            ...
            {
                bFlag = false;
                Dialog.OpenDialogInAjax(txtOrganNo, "车间编号不能为空！");
            }
            else if (SysClass.SysOrgan.CheckOrganNoExists(_OrganID, txtOrganNo.Text))
            {
                bFlag = false;
                Dialog.OpenDialogInAjax(txtOrganName, "车间编号不能重复！");
            }
            else if (txtOrganName.Text.Length == 0)
            {
                bFlag = false;
                Dialog.OpenDialogInAjax(txtOrganName, "车间名称不能为空！");
            }
            return bFlag;
        }
        ...
        protected void btnApply_Click(object sender, EventArgs e)
        {
            if (SaveCheck())
            {
                ...
                {
                    Dialog.OpenDialogInAjax(upForm, "恭喜您，保存信息成功……", "Organ_Lst.aspx?POrganID=" + _POrganID.ToString());
                }
            }
        }
        ...
        {
            Response.Redirect("Organ_Lst.aspx?POrganID=" + _POrganID.ToString());
        }   
    }
}
