<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Dept_Lst.aspx.cs" Inherits="JtgTMS.Admin.Dept_Lst" %>
...
<form id="upForm" name="ctl00" class="form-inline" runat="server">
  ...
  <ul class="breadcrumb" style="display:none"><li>系统管理 <span class="divider">/</span></li><li>部门管理 <span class="divider">/</span></li><li class="active"><% =_OrganName %></li></ul>
  ...
  <asp:GridView ID="gvLists" name="contents" runat="server" CssClass="table table-bordered info table-hover"  GridLines="none"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="">
                <ItemTemplate>
                    <a id="hyEdit" href='../Admin/Dept_Edit.aspx?OrganID=<%# Eval("ID").ToString()%>&POrganID=<%# Eval("POrganID").ToString()%>'>编辑</a>
                </ItemTemplate>
                <ItemStyle CssClass="center" Width="6%" />
            </asp:TemplateField>
            <asp:TemplateField HeaderText="">
                <ItemTemplate>
                    <a id="hyEdit" href='../Admin/Dept_Lst.aspx?DeleteOrganID=<%# Eval("ID").ToString()%>&POrganID=<%# Eval("POrganID").ToString()%>' style='display:<%# Eval("ID").ToString()=="58" ? "None":"none1"%>' onclick="return confirm('此操作将删除所选的部门，确认吗？')">删除</a>
                </ItemTemplate>
                <ItemStyle CssClass="center" Width="6%" />
            </asp:TemplateField>
             <asp:TemplateField HeaderText="">
            ...
            <ItemTemplate>               
                <asp:CheckBox ID="CheckRow" name="IDsCollection" Visible='<%# Eval("ID").ToString()=="58" ? false:true%>' runat="server" ToolTip="选中/取消" />
            </ItemTemplate>
            <ItemStyle Width="3%"/>
        </asp:TemplateField>
        </Columns>
  ...
  <ul class="breadcrumb breadcrumb-button">    
    <asp:Button ID="btnAdd" Text="添加部门" CssClass="btn" runat="server" 
          onclick="btnAdd_Click" />
    <asp:Button ID="btnDelete" CssClass="btn" Text="删 除" OnClientClick="return confirm('此操作将删除所选的部门，确认吗？');" onclick="btnDelete_Click" runat="server" />
  </ul>
