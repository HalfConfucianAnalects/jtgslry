...
UE.plugins['video'] = function (){
     ...
     */
    function creatInsertStr(url,width,height,isAutoPlay,align,toEmbed,addParagraph){
        return  !toEmbed ?
                (addParagraph? ('<p '+ (align !="none" ? ( align == "center"? ' style="text-align:center;" ':' style="float:"'+ align ) : '') + '>'): '') +
                '<img align="'+align+'" width="'+ width +'" height="' + height + '" _url="'+url+'" _isAutoPlay="' + isAutoPlay + '" class="edui-faked-video"' +
                ...
                (addParagraph?'</p>':'')
                :
				'<embed class="edui-faked-video" playurl="' + url + '" width="' + width + '" height="' + height + '" isAutoPlay="' + isAutoPlay + '"></embed>';
...
//                ' wmode="transparent" play="true" loop="false" menu="false" allowscriptaccess="never" allowfullscreen="true" >';
    ...
    function switchImgAndEmbed(img2embed){
        ...
        for(var i=0,node;node = nodes[i++];){
            ...
            tmpdiv.innerHTML = creatInsertStr(img2embed ? node.getAttribute("_url"):node.getAttribute("playurl"),node.width,node.height, img2embed ? node.getAttribute("_isAutoPlay"):node.getAttribute("isAutoPlay"),align || node.getAttribute("align"),img2embed);
    ...
    me.commands["insertvideo"] = {
        execCommand: function (cmd, videoObjs){
            ...
            for(var i=0,vi,len = videoObjs.length;i<len;i++){
                 vi = videoObjs[i];
                 html.push(creatInsertStr( vi.url, vi.width || 420,  vi.height || 280, vi.isAutoPlay ? vi.isAutoPlay : "true", vi.align||"none",false,true));
            }
            me.execCommand("inserthtml",html.join(""));
        },
        queryCommandState : function(){
...
UE.plugins['music'] = function () {
    ...
    function creatInsertStr(url,width,height,isAutoPlay,align,toEmbed,addParagraph){
        return  !toEmbed ?
            (addParagraph? ('<p '+ (align !="none" ? ( align == "center"? ' style="text-align:center;" ':' style="float:"'+ align ) : '') + '>'): '') +
                '<img align="'+align+'" width="'+ width +'" height="' + height + '" _url="'+url+'" _isAutoPlay="'+isAutoPlay+'" class="edui-faked-music"' +
                ' src="'+me.options.langPath+me.options.lang+'/images/music.png" />' +
                (addParagraph?'</p>':'')
            :
            '<embed class="edui-faked-music"' +
                ' playUrl="' + url + '" width="' + width  + '" height="' + height  + '" isAutoPlay="' + isAutoPlay  + '" align="' + align + '"' +
                ( align !="none" ? ' style= "'+ ( align == "center"? "display:block;":" float: "+ align )  + '"' :'' ) +
                '></embed>';
    }
    ...
    function switchImgAndEmbed(img2embed) {
        ...
        for (var i = 0, node; node = nodes[i++];) {
            ...
            //先看float在看align,浮动有的是时候是在float上定义的
            var align = node.style.cssFloat;
            tmpdiv.innerHTML = creatInsertStr(img2embed ? node.getAttribute("_url") : node.getAttribute("playUrl"), node.width, node.height, img2embed ? node.getAttribute("_isAutoPlay") : node.getAttribute("isAutoPlay"), node.getAttribute("align"), img2embed);
            node.parentNode.replaceChild(tmpdiv.firstChild, node);
        }
    }
    ...
    me.commands["music"] = {
        execCommand:function (cmd, musicObj) {
            var me = this,
                str = creatInsertStr(musicObj.url, musicObj.width || 400, musicObj.height || 95, musicObj.isAutoPlay ? musicObj.isAutoPlay : "true", "none", false, true);
