<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Role_Lst.aspx.cs" Inherits="JtgTMS.Admin.Role_Lst" %>
...
<form id="upForm" name="ctl00" class="form-inline" runat="server">
  ...
  <ul class="breadcrumb" style="display:none"><li>系统管理 <span class="divider">/</span></li><li>用户管理 <span class="divider">/</span></li><li class="active">角色</li></ul>
  ...
  <asp:GridView ID="gvLists" name="contents" runat="server" CssClass="table table-bordered info table-hover"  GridLines="none"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="">
                <ItemTemplate>
                    <a id="hyEdit" href='../Admin/Role_Edit.aspx?RoleID=<%# Eval("ID").ToString()%>'>编辑</a>
                </ItemTemplate>
                <ItemStyle CssClass="center" Width="6%" />
            </asp:TemplateField>
            <asp:TemplateField HeaderText="">
                <ItemTemplate>
                    <a id="hyEdit" href='../Admin/Role_Lst.aspx?DeleteRoleID=<%# Eval("ID").ToString()%>' style='display:<%# Eval("ID").ToString()=="30" ? "None":"none1"%>' onclick="return confirm('此操作将删除所选的角色，确认吗？')">删除</a>
                </ItemTemplate>
                <ItemStyle CssClass="center" Width="6%" />
            </asp:TemplateField>
             <asp:TemplateField HeaderText="">
                ...
                <ItemTemplate>               
                    <asp:CheckBox ID="CheckRow" Visible='<%# Eval("ID").ToString()=="30" ? false:true%>' name="IDsCollection" runat="server" ToolTip="选中/取消" />
                </ItemTemplate>
            <ItemStyle Width="3%"/>
        </asp:TemplateField>
        </Columns>
  ...
  <ul class="breadcrumb breadcrumb-button">    
    <asp:Button ID="btnAdd" Text="添加角色" CssClass="btn" runat="server" 
          onclick="btnAdd_Click" />
    <asp:Button ID="btnDelete" CssClass="btn" Text="删 除" OnClientClick="return confirm('此操作将删除所选的角色，确认吗？');" onclick="btnDelete_Click" runat="server" />
  </ul>
