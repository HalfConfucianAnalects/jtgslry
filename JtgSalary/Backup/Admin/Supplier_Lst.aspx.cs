...
{
    public partial class Supplier_Lst : System.Web.UI.Page
    {
        public int _PSupplierID = 0, _DeleteSupplierID = 0;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.Params["DeleteSupplierID"] != null)
            {
                _DeleteSupplierID = int.Parse(Request.Params["DeleteSupplierID"]);
            }
            if (!Page.IsPostBack)
            {
                BindPageData();
        ...
        {
            if (_DeleteSupplierID > 0)
            {
                ///执行删除操作
                SysClass.SysOrgan.DeleteSingleOrgan(_DeleteSupplierID.ToString());
            }

            _SupplierName = SysClass.SysOrgan.GetOrganNameByID(_PSupplierID);
            this.PageInfo.InnerHtml = SysClass.SysPageNums.GetPageRawUrlNum(SysClass.SysOrgan.GetOrganLstByDataSet(_PSupplierID, _OrganType, txtSearchKeyword.Text), gvLists, 15);
        }        
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                BindPageData();
                Dialog.OpenDialogInAjax(txtSearchKeyword, "恭喜您，" + _SupplierName + "选择机构部门删除成功……");
            }
        ...
        {
            Response.Redirect("Supplier_Edit.aspx?PSupplierID=" + _PSupplierID.ToString() + "");
        }
